    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['GET:/system/post/export']"
        >导出</el-button>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.role_id).join(",")
    ...
    handleDataScope(row) {
      this.reset();
      this.$nextTick(() => {
        this.getRoleDeptTreeselect(row.role_id);
      });
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.role_id != undefined) {
            this.form.menuIds = this.getMenuAllCheckedKeys().join(",");
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const roleIds = row.role_id || this.ids;
