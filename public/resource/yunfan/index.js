...
$(function() {
    ...
    function setActiveTab(element) {
        if (!$(element).hasClass('active')) {
            ...
            // 显示tab对应的内容区
            $('.YunFan_iframe').each(function() {
                if ($(this).data('id') == currentId) {
                    $(this).show().siblings('.YunFan_iframe').hide();
                }
            });
    ...
    function menuItem() {
        ...
        $('.menuTab').each(function() {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    scrollToTab(this);
                    // 显示tab对应的内容区
                    $('.mainContent .YunFan_iframe').each(function() {
                        if ($(this).data('id') == dataUrl) {
                            $(this).show().siblings('.YunFan_iframe').hide();
                            return false;
                        }
        ...
        if (flag) {
            ...
            // 添加选项卡对应的iframe
            var str1 = '<iframe class="YunFan_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
            $('.mainContent').find('iframe.YunFan_iframe').hide().parents('.mainContent').append(str1);
    ...
    function refreshTab() {
    	...
    	var target = $('.YunFan_iframe[data-id="' + currentId + '"]');
    ...
    function tabCloseOther() {
        $('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function() {
            $('.YunFan_iframe[data-id="' + $(this).data('id') + '"]').remove();
            $(this).remove();
        });
    ...
    function tabCloseAll() {
        ...
        $('.page-tabs-content').children("[data-id]:first").each(function() {
            $('.YunFan_iframe[data-id="' + $(this).data('id') + '"]').show();
    ...
    $.contextMenu({
        ...
        items: {
            ...
            "full": {
                ...
                callback: function(key, opt) {
                	setActiveTab(this);
                    var target = $('.YunFan_iframe[data-id="' + this.data('id') + '"]');
                	target.fullScreen(true);
                }
            },
            "refresh": {
